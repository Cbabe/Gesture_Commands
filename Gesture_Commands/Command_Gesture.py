...
class gesture_command(Node):
   ...
   def process_image(self):
       with self.mp_hands.Hands(model_complexity=0, min_detection_confidence=0.5,min_tracking_confidence=0.5) as hands: # Initializes hands model
   ...
   def drive_square(self):
       ...
       for i in range(4): # Repeat for each side
           ...
           while math.sqrt((self.pose[0]-start_x)**2 + (self.pose[1] -start_y)**2) < side: # Print what side of the square the Neato is driving
               if i == 0:
           ...
           msg.angular.z = 0.75
           self.vel_pub.publish(msg)
           while abs(self.pose[2] - start_theta) < 90: # Print what turn the Neato is driving
               if i == 0:
   ...
   def drive_triangle(self):
       ...
       for i in range(3):
           ...
           msg.angular.z = 0.0
           self.vel_pub.publish(msg)
           while math.sqrt((self.pose[0]-start_x)**2 + (self.pose[1] -start_y)**2) < side: # Print what side of the triangle the Neato is driving
               if i == 0:
           ...
           msg.angular.z = 0.75
           self.vel_pub.publish(msg)
           while abs(self.pose[2] - start_theta) < 120: # Print what turn the Neato is driving
               if i == 0:
                   print("driving first turn..")
               if i == 1:
                   print("driving second turn...")
