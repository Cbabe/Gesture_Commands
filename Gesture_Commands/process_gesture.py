...
class GestureIdentifier(Node):

    def __init__(self, image_topic):
        """ Initialize the object tracker """
        super().__init__('gesture_identifier')
...
if __name__ == '__main__':
    node = GestureIdentifier("/camera/image_raw") # not sure if the input path should stay
    node.run()

def main(args=None):
    rclpy.init()
    n = GestureIdentifier("camera/image_raw")
    rclpy.spin(n)
    rclpy.shutdown()
