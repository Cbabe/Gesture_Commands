...
Run the file: ```Gesture_Commands/Command_Gesture.py```
### To run Draw:
Run the file: ```Gesture_Commands/Draw.py```
### To Retrain the Cass Model:
Run the file: ```Gesture_Commands/custom_gestures/classify_gestures.ipynb```
# Report
## Phase 1
### Our Idea
...
Starting with hand tracking, we identify the tip of the finger using MediaPipe and record all the positions of this element with time. Plotting all the locations on a single image will then allow us to essentially connect the dots and “trace” the path the finger has drawn and obtain the required shape.<br/>
...
## Conclusion
As a team and individuals we are very satisfied with our work. Our MVP was achieved and we all learned more about computer vision and algorithms as intended. Team goals were also met as communication was very good during this meeting and we were attentive about splitting work well and knowing when we needed to come together to work on something. While we would have liked to get the custom path drawing working, we were also able to figure out how to actually achieve that which is very satisfying, the issue was simply a lack of time.
 
## Sources
1. [Machine Learning Model](https://github.com/kinivi/hand-gesture-recognition-mediapipe)
2. [Media Pipe](https://google.github.io/mediapipe/)
